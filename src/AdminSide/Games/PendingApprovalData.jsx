...
import { Nav, Form, Image, Button } from "react-bootstrap";
...
export default function PendingApprovalData() {
    ...
    const { AllGames, uploadedGames } = useContext(GameContext)
    ...
    const selectedGame = AllGames?.find(
        (g) => g?.title?.en?.toLowerCase() === gameTitle?.toLowerCase()
    );

    // =====================================
    // USE THIS HELPER AT TOP OF COMPONENT
    // =====================================
    const getValue = (field, defaultValue = "") => {
        return gameById?.pendingChanges?.[field] ??
            gameById?.[field] ??
            defaultValue;
    };

    const { gameById, uploadedCacheGames, setGameById, purgeGameCache } =
        ...
        <>
            ...
            {selectedLanguages[field]?.map((lang) => (
                <Form.Control
                    ...
                    value={
                        gameById?.pendingChanges?.[field]?.[lang]
                        ?? gameById?.[field]?.[lang]
                        ?? ""
                    }
        ...
        <Form onSubmit={updateGameFn}>
            ...
            {activeTab === "form" && (
                <>
                    ...
                    <ReactQuill value={getValue("controls")} onChange={handleControlsChange} />
                    <Form.Label>How To Play *</Form.Label>
                    <ReactQuill value={getValue("howToPlay")} onChange={handleHowPlayChange} />
                    <Form.Label>Feature List *</Form.Label>
                    <ReactQuill value={getValue("featureList")} onChange={handleFeatureListChange} />
                    ...
                    {/* FAQ */}
                    {gameById?.pendingChanges?.faqs?.map((faq, i) => (
                        <div key={i}>
                            <Form.Control
                                className="mb-1"
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control as="select" name="featureGame" value={getValue("featureGame")} onChange={onchange}>
